...
const handleSubmit = async (e) => {
  ...
  // Alternative approach for both Register and Login pages
const handleCivicSignIn = async () => {
  try {
    setAuthState('signing-in');
    console.log("Starting Civic sign-in process");
    
    // First, clear any existing Civic session
    if (window.civic) {
      try {
        await window.civic.logout();
      } catch (e) {
        console.log("No existing session to clear");
      }
    }
    
    // Now sign in with prompt parameter
    await signIn({
              ...
              {registrationSuccess && (
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mt-1">Your account has been created. Please sign in to continue.</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>

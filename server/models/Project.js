...
projectSchema.pre('save', function(next) {
  ...
  // Set a flag for NEW projects to ensure matching runs
  if (this.isNew) {
...
projectSchema.post('save', async function(doc, next) {
  try {
    // CRITICAL: Use the flag we set in pre-save to detect new projects
    if (doc._isNewProject && doc.neededRoles && doc.neededRoles.length > 0) {
      ...
      // Import dependencies inside the hook to avoid circular dependencies
      const { matchUsersToProject } = require('../utils/matching');
      const Notification = require('./Notification');
      ...
      setTimeout(async () => {
        try {
          ...
          // Only create notification if we have valid matches
